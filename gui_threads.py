...
class updateQTTargetThread3D(QThread):
    ...
    def getPointColors(self, i):
        if self.pointBounds["enabled"]:
            ...
            if (
                xyz_coords[0] < self.pointBounds["minX"]
        ...
        # Color the points by their associate track
            ...
            if (
                trackIndex == TRACK_INDEX_WEAK_SNR
                or trackIndex == TRACK_INDEX_BOUNDS
                or trackIndex == TRACK_INDEX_NOISE
            ):
                return pg.glColor("w")
            else:
