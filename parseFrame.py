...
def parseStandardFrame(frameData):
    ...
    try:
        (
            ...
            numTLVs,
            subFrameNum,
        ) = struct.unpack(headerStruct, frameData[:frameHeaderLen])
    except:
    ...
    for i in range(numTLVs):
        ...
        if tlvType == MMWDEMO_OUTPUT_MSG_DETECTED_POINTS:
            outputDict["numDetectedPoints"], outputDict["pointCloud"] = (
                parsePointCloudTLV(
        ...
        # Range Profile
            ...
            outputDict["rangeProfileMajor"] = parseRangeProfileTLV(
                frameData[:tlvLength]
            )
        # Range Profile
        elif tlvType == MMWDEMO_OUTPUT_EXT_MSG_RANGE_PROFILE_MINOR:
            outputDict["rangeProfileMinor"] = parseRangeProfileTLV(
                frameData[:tlvLength]
            )
        # Noise Profile
        ...
        # Side Info
            ...
            outputDict["pointCloud"] = parseSideInfoTLV(
                frameData[:tlvLength], tlvLength, outputDict["pointCloud"]
            )
        ...
        # Spherical Points
            ...
            outputDict["numDetectedPoints"], outputDict["pointCloud"] = (
                parseSphericalPointCloudTLV(
                    frameData[:tlvLength], tlvLength, outputDict["pointCloud"]
                )
            )
        # Target 3D
        elif (
            tlvType == MMWDEMO_OUTPUT_MSG_TRACKERPROC_3D_TARGET_LIST
            or tlvType == MMWDEMO_OUTPUT_EXT_MSG_TARGET_LIST
        ):
            outputDict["numDetectedTracks"], outputDict["trackData"] = parseTrackTLV(
                frameData[:tlvLength], tlvLength
            )
        elif tlvType == MMWDEMO_OUTPUT_MSG_TRACKERPROC_TARGET_HEIGHT:
            outputDict["numDetectedHeights"], outputDict["heightData"] = (
                parseTrackHeightTLV(frameData[:tlvLength], tlvLength)
            )
        # Target index
        elif (
            tlvType == MMWDEMO_OUTPUT_MSG_TRACKERPROC_TARGET_INDEX
            or tlvType == MMWDEMO_OUTPUT_EXT_MSG_TARGET_INDEX
        ):
            outputDict["trackIndexes"] = parseTargetIndexTLV(
                frameData[:tlvLength], tlvLength
            )
        # Capon Compressed Spherical Coordinates
        elif tlvType == MMWDEMO_OUTPUT_MSG_COMPRESSED_POINTS:
            outputDict["numDetectedPoints"], outputDict["pointCloud"] = (
                parseCompressedSphericalPointCloudTLV(
                    frameData[:tlvLength], tlvLength, outputDict["pointCloud"]
                )
            )
        # Presence Indication
        ...
        # Occupancy State Machine
            ...
            outputDict["gestureNeuralNetProb"] = parseGestureProbTLV6843(
                frameData[:tlvLength]
            )
        elif (
            tlvType == MMWDEMO_OUTPUT_MSG_GESTURE_FEATURES_6432
        ):  # 6432 features output 350
            outputDict["gestureFeatures"] = parseGestureFeaturesTLV6432(
                frameData[:tlvLength]
            )
        ...
        # Performance Statistics
            ...
            (
                outputDict["procTimeData"],
                outputDict["powerData"],
                outputDict["tempData"],
        ...
        # Presence Detection in each zone
            ...
            outputDict["enhancedPresenceDet"] = parseEnhancedPresenceInfoTLV(
                frameData[:tlvLength], tlvLength
            )
        # Probabilities output by the classifier
        elif tlvType == MMWDEMO_OUTPUT_EXT_MSG_CLASSIFIER_INFO:
            outputDict["classifierOutput"] = parseClassifierTLV(
                frameData[:tlvLength], tlvLength
            )
        ...
        # Surface classification value
            ...
            outputDict["surfaceClassificationOutput"] = parseSurfaceClassificationTLV(
                frameData[:tlvLength]
            )
        else:
